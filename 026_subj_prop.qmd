...
```{r fig-prior-bino-12-20-1, fig.cap="Distribuzione a priori per il parametro $\\theta$ (versione 1)."}
tibble(theta, p1) %>% 
  ggplot(aes(theta, p1)) +
  geom_segment(
...
```{r fig-prior-bino-12-20-2, fig.cap="Distribuzione a priori per il parametro $\\theta$ (versione 2)."}
tibble(theta, p2) %>% 
  ggplot(aes(theta, p2)) +
  geom_segment(
    aes(xend = theta, yend = 0), size = 10, lineend = "butt"
...
```{r fig-like-12-20, fig.cap="Verosimiglianza del modello binomiale nel caso di 12 successi in 20 prove."}
tibble(theta, like) %>% 
...
La distribuzione a posteriori di $\theta$ sarà dunque uguale a `(p2 * like) / sum(p2 * like)`.
...
```{r fig-post-bino-12-20-discr, fig.cap="Distribuzione a posteriori per il modello binomiale con 12 successi in 20 prove e la distribuzione a priori indicata in  @fig-prior-bino-12-20-2."}
tibble(theta, post) %>% 
